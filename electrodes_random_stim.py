...
class RandomStimElectrodes:
    def __init__(self, ch_coordinates : np.ndarray, stim_pulse_params: dict, current_disperse_func: callable):
        """
    ...
    def generate_random_stimulation(self, channels, amplitude_range, duration_ms, stim_rate_hz=10, min_pulse_spacing=1, n_groups=1):
        """
        ...
        for group in channels_per_group:
            ...
            self.stim_onset_times_by_ch[group[0]] = []  # Initialize for the first channel in the group
            ...
            for stim_time in stim_times:
                ...
                self.stim_onset_times_by_ch[group[0]].append(times[0])  # Store onset time for the first channel
            
            # Apply the same stimulation to all channels in the group
            for ch in group:
                self.stim_onset_times_by_ch[ch] = self.stim_onset_times_by_ch[group[0]].copy()  # Copy times to other channels
                stimulations.append({
                    'channel': ch,
    ...
    def get_current_at_locs(self, locs):
        ...
        for ch_idx, stim in enumerate(self.stimulations):
            ch_current_vals = np.array([amp for _, amps in stim['pulse_params'] for amp in amps])  # All amplitudes
        ...
        for i_loc in range(n_locs):  # For each neuron
            ...
            for ch_idx, stim in enumerate(self.stimulations):
                current_times_ch = np.array([time for times, _ in stim['pulse_params'] for time in times])
                current_vals_ch = currents_ch2loc[ch_idx][i_loc, :]  # Get current values at this location
    ...
    def plot_stim_raster(self, time_range_ms=None, ax=None):
        ...
        for i, ch_idx in enumerate(stim_channel_indices):
            ...
            ax.eventplot(stim_train, lineoffsets=i + 1, linelengths=0.5, colors="k")  # Plot stim trains
